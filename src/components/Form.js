    ...
    return (
        <Forms onSubmit={handleSubmit} disabled={disabled}>
            <fieldset disabled={disabled}>
                {type === "register" ?
                    <input type="text" id="name" pattern={regEx.name} placeholder="Name" required value={name} onChange={e => setName(e.target.value)} />
                    : null}
                <input type="email" id="email" pattern={regEx.email} placeholder="E-mail" required value={email} onChange={e => setEmail(e.target.value)} />
                <input type="password" id="password" pattern={regEx.password} placeholder="Password" required value={password} onChange={e => setPassword(e.target.value)} />
                {type === "register" ?
                    <input type="password" id="passwordConfirmation" pattern={regEx.password} placeholder="Confirm password" required value={passwordConfirmation} onChange={e => setPasswordConfirmation(e.target.value)} />
                    : null}
                <button type="submit">{disabled ? "Loading..." : (type === "register" ? "Register" : "Login")}</button>
            </fieldset>
        </Forms>
    )
}
